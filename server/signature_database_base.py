...
class SignatureDatabaseBase(object):
    ...
    def add_image_signature(self, path, signature, metadata=None, refresh_after=False):
        """same as add_image, but we already have signature this time
        """
        rec = make_record(path, self.gis, self.k, self.N, signature=signature, metadata=metadata)
        self.insert_single_record(rec, refresh_after=refresh_after)

    def search_image(self, path, image=None, all_orientations=False, bytestream=False, pre_filter=None):
        """Search for matches
        Args:
...
def make_record(path, gis, k, N, signature=None, img=None, bytestream=False, metadata=None):
    ...
    Args:
        ...
        signature(Optional): the signature of the image
        img (Optional[string]): usually raw image data. In this case, path will still be stored, but
            a signature will be generated from data in img. If bytestream is False, but img is
            not None, then img is assumed to be the URL or filesystem path. Thus, you can store
            image records with a different 'path' than the actual image location (default None)
